...
namespace assets
{
	...
	public:
		...
		FileData(const std::byte* a_data, uint64_t a_size)
			: data(a_data), size(a_size) {}

		// backed by mmap which is kept open for this objects lifetime
		FileData(const std::byte* a_data, uint64_t a_size, zenkit::Mmap&& a_mmap)
			: data(a_data), size(a_size), mmap(std::move(a_mmap)) {}

		// backed by heap buffer which is kept for this objects lifetime
		FileData(const std::byte* a_data, uint64_t a_size, std::unique_ptr<std::vector<std::uint8_t>>&& a_buffer)
			: data(a_data), size(a_size), buffer(std::move(a_buffer)) {}
